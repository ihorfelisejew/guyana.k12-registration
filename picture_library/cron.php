...
$sql = "SELECT * FROM `rounds` WHERE `round` = '1'";
...
if(! $retval )
	{
		...
		$sql1 = "SELECT * FROM `challenge` WHERE `round` = '1'";
				...
				while($row6 = mysqli_fetch_assoc($retval6)){
					...
					while($row8 = mysqli_fetch_assoc($retval8))
					{
						if($i == 1){
							...
							$sql9 = "SELECT * FROM `winners` WHERE `round` LIKE '1' AND `team_id` LIKE '$team_id' AND `score` = '$score' AND `county` LIKE '$county1'" ;
							...
							if ($numResults > 0) {
							   ...
							   // no data from query, react accordingly
								echo $team_id.', Score:'.$score.', County: '.$county1.'<br/> ';
								$sql5 = "INSERT INTO `winners`(`round`, `team_id`, `score`, `county`) VALUES ('1','$team_id', '$score','$county1')";
								...
								while($row10 = mysqli_fetch_assoc($retval10)){
									...
									$message2 ="Congrats your team $teamname is the winner of round one and will  now move on the STATE championship for round 2";
								...
								while($row11 = mysqli_fetch_assoc($retval11)){
									...
									$message2 ="Congrats your team $teamname2 is the winner of round one and will  now move on the STATE championship for round 2";
