...
$sql = "SELECT * FROM `rounds` WHERE `round` = '3'";
...
if(! $retval )
	{
		...
		$sql1 = "SELECT * FROM `challenge` WHERE `round` = '3'";
			...
			//echo 'hi';
					...
					while($row8 = mysqli_fetch_assoc($retval8))
					{
						if($i == 1){
							...
							$sql9 = "SELECT * FROM `winners3` WHERE `round` LIKE '3' AND `team_id` LIKE '$team_id' AND `score` = '$score' AND `county` LIKE '$county1'" ;
							...
							if ($numResults > 0) {
							   ...
							   // no data from query, react accordingly
								...
								$sql5 = "INSERT INTO `winners3`(`round`, `team_id`, `score`, `county`, `state`) VALUES ('3','$team_id', '$score','$county1', '$state')";
