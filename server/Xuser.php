...
function login($data){
    ...
    if($phone != ''){
        $sql = "SELECT * FROM `users` where phone = '$phone' and password = '$password' and role_id = '2'";
    }else{
...
function saveOrganization($data){
    ...
    if($check_exist < 1){
        $sql_qii = "INSERT INTO users (`name`, `last_name`, `email`, `password`, `role_id`, `org_name`, `county`, `city`, `state`, `school_type`, `phone`, `parent_consent`, `parent_email`, `zip`, `league`, `picUrl`, `league_id`, `status`, `org_id`)
								VALUES ('$firstname', '$lastname','$orgemail','$orgpass', '4', '$orgname', '$orgcounty', '', '$orgstate', '$school_type', '$orgphoneno', '0', '' ,'$zip', '', '', '','1','0' )";
