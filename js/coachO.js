...
$(document).ready(function() {
	...
	if(user != undefined && user != ""){
		...
		$("#chat-user").val(user.id);
		var user = { user_id : user.id} ;
		$.ajax({
			type : 'POST',
			...
			//	alert(user.id);
			  user1 = JSON.parse(response);
			 //  alert(user1.t_id);
			  
			  $("#chat-team").val(user1.t_id);
			   // document.getElementsByName('#chat_user_id').value = userid;
			  //document.getElementsByName('#chat_team_id').value = user.t_id;
